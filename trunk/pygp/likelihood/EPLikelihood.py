...
def sigmoid(x):
    ...
    return ( 1+special.erf(x/SP.sqrt(2.0)) )/2.0
def gos(x):
    """Gaussian over sigmoid"""
    return ( SP.sqrt(2.0/SP.pi)*SP.exp(-0.5*x**2)/(1+special.erf(x/SP.sqrt(2.0))) )
...
class GaussLikelihood(ALikelihood):
    ...
    def calcExpectations(self,t,cav_np,x=None):
        ...
        Z  = norm(cav_mp[0],SP.sqrt(stot)).pdf(t)
...
class MOGLikelihood(ALikelihood):
    ...
    def setLogtheta(self,logthetaL):
        ...
        #split in phi/sl
        self.phi = SP.exp(logthetaL[0:np_2])
        self.sl = SP.exp(2*logthetaL[np_2::])
    ...
    def calcExpectations(self,t,cav_np,x=None):
        ...
        for c in range(self.Ncomponents):
            ...
            #0. moment
            dc[0] = self.phi[c]*norm(cav_mp[0],SP.sqrt(Sc)).pdf(t)
